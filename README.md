...
| 数据库 | SQLite |
| LLM | DeepSeek / OpenAI / 小米 MiMo 等兼容 API |
...
在「系统设置 → 模型配置」中选择模型提供方。小米 MiMo 可直接选择 `mimo`：

- Base URL：`https://api.xiaomimimo.com/v1`
- Model：`mimo-v2.5-pro`
- API Key：使用前在界面输入，不写入默认配置

后端会按 MiMo OpenAI 兼容接口发送 `max_completion_tokens`，并默认关闭 thinking，便于规则抽取稳定返回 JSON。
